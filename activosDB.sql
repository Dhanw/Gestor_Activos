...
CREATE TABLE IF NOT EXISTS `mydb`.`Usuarios` (
  ...
  PRIMARY KEY (`ID`),
  UNIQUE INDEX `nombre_cuenta_UNIQUE` (`cuenta` ASC) VISIBLE,
  INDEX `fk_Usuarios_Funcionarios1_idx` (`funcionario` ASC) VISIBLE,
  UNIQUE INDEX `Funcionarios_ID_UNIQUE` (`funcionario` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`Solicitudes` (
  `ID` INT(5) NOT NULL AUTO_INCREMENT,
  `codigo_comprobante` VARCHAR(15) NOT NULL,
  `fecha_adquisicion` DATE NOT NULL,
  `tipo_adquisicion` INT(1) NOT NULL,
  `estado` INT(1) NOT NULL,
  `registrador` INT(5) NULL,
  PRIMARY KEY (`ID`),
  UNIQUE INDEX `codigo_UNIQUE` (`codigo_comprobante` ASC) VISIBLE,
  INDEX `fk_Solicitudes_Usuarios_idx` (`registrador` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`Activos` (
  ...
  INDEX `fk_Activos_Dependencias1_idx` (`dependencia` ASC) VISIBLE,
  INDEX `fk_Activos_Funcionarios1_idx` (`encargado` ASC) VISIBLE,
...
insert into Dependencias(nombre,ubicacion) values('Escuela de informatica','Lagunilla-Heredia');
insert into Dependencias(nombre,ubicacion) values('Escuela de matematica','Heredia Centro');
