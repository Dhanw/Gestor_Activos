...
public class TestHABasics extends BaseFabricTestCase {
    ...
    /**
     * Test that new connections get the state changes from the Fabric node. The
     * current implementation queries the Fabric node on every new connection
     * and therefore always has the latest state. Future shared state
     * implementation may loosen this constraint.
     */
    ...
    /**
     * Test that partially failed over connections (those that failed but could
     * not immediately get a new connection) don't impact the creation of new
    ...
    /**
     * Same as test `manualTestFailedOldMasterDoesntBlockNewConnections' for
     * slaves. There must be only one slave in the HA group for this test.
     */
