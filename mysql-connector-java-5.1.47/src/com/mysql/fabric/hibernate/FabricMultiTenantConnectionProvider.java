...
public class FabricMultiTenantConnectionProvider implements MultiTenantConnectionProvider {
    ...
    /**
     * Find a server with mode READ_WRITE in the given server group and create a
     * JDBC connection to it.
     *
     * @returns a {@link Connection} to an arbitrary MySQL server
     * @throws SQLException if connection fails or a READ_WRITE server is not
    ...
    /**
     * Get a connection that be used to access data or metadata not specific to
     * any shard/tenant. The returned connection is a READ_WRITE connection to
     * the global group of the shard mapping for the database and table
     * association with this connection provider.
     */
    ...
    /**
     * Get a connection to access data association with the provided
     * `tenantIdentifier' (or shard key in Fabric-speak). The returned
     * connection is a READ_WRITE connection.
     */
